...
namespace BriskDesk.Services
{
    ...
    public static class StaticRandomNumberGenerator
    {
        ...
        /// </summary>		
        /// <returns>A 32-bit unsigned integer greater than or equal to zero and less than UInt32.MaxValue.</returns>
        ...
        public static double NextUInt32()
        {
            lock (myLock)
            {
                return (Math.Truncate(random.NextDouble() * (UInt32.MaxValue - UInt32.MinValue)));
            }
        }
        ...
        public static bool TrueWithProbability(int percentage)
        {
            int rand = StaticRandomNumberGenerator.Next(1, 100);
